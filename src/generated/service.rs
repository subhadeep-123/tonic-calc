...
pub mod calculator_server {
    ...
    #[async_trait]
    pub trait Calculator: std::marker::Send + std::marker::Sync + 'static {
        async fn add(
            &self,
    ...
    impl<T> Clone for CalculatorServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "service.Calculator";
    impl<T> tonic::server::NamedService for CalculatorServer<T> {
